...
- Updated the Hebrew and Arabic lesson taglines.
...
- Restored the App Router's single root document shell, preventing opaque React Server Components errors during localized navigation.
...
- Suppressed detail-free React Server Components wrapper errors after their server cause is reported.
...
- Tag server-render errors with their digest and rendering source in GlitchTip.
...
- Restored topic question counts on legacy databases that lack the questions activity column.
...
- Restored the missing question-report table through an idempotent migration, preserving existing report schemas.
...
- Restored topic-accuracy reads for databases that predate `questions.is_active`.
...
- Aligned every existing page-loading skeleton with its loaded page's cards, controls, media, and navigation layout.
...
- Normalized historical release headings to inferred SemVer and enforced a single stable SemVer increment with an inserted, matching top-level changelog entry for each PR; prerelease and build-metadata versions are rejected.
...
- Polished the mobile Progress page with a featured readiness action and compact learner metrics.
...
- Added a guarded QA-to-production migration release pipeline, Vercel schema-ledger gate, and QA active-content schema probes.
...
- Consolidated API request translation and server-client setup while retaining route-owned authorization responses.
...
- Removed unused library helpers, orphaned tests, and unreferenced maintenance scripts; retained the Arabic question fixture only for tests.
...
- Added a Hebrew question-bank audit that verifies the live questions, options, and correct answers against the Ministry XML.
...
- Added reviewed, source-pinned content release validation for Ministry questions and Israeli traffic signs, with active-content cutover safeguards.

### Fixed

- Prevented concurrent exam creation and invalidated unfinished exams at content cutover so retired questions cannot be finalized.
...
- Restored the referenced prompt signs for audited traffic-sign questions in deployments with stale question data.
...
- Aligned bottom navigation and topic-card icon sizing with the More-page standard, while retaining traffic-sign artwork.
...
- Removed the duplicate exam-simulation link from More; the bottom navigation remains the single entry point.
...
- Restored the shared bottom navigation on schedule, topic learning and review flows, diagnostic, loading states, and the localized error screen.
...
- Replaced blank saved-question, mistake-review, and unavailable-question views with localized, actionable empty-state cards.
...
- Restored More to the five-tab navigation, moved mistake review into Practice, and combined video lessons and useful links under Resources. Legacy video URLs now redirect to Resources.
...
- Localized landing-page screenshots now match the active language and theme, with refreshed dashboard, quiz, and flashcard previews.
...
- Restored test coverage above 95% across lines, statements, functions, and branches with targeted page and API-path tests.
...
- Added authenticated question reporting from practice and retry questions. (#268)
...
- Moved flashcards before videos in the shared bottom navigation.
...
- More now has account-focused copy, persistent semantic settings controls, protected language switching, and a destructive logout action. (#250)
...
- Bookmarks now uses the shared More navigation and removes redundant close and return-home controls. (#251)
...
- Credits now uses the shared More navigation, removes redundant header/footer chrome, and links to the live Wikimedia road-sign category. (#252)
...
- Exam start now uses the shared bottom navigation with Home active, without a redundant close control. (#253)
...
- Flashcards now use the shared bottom navigation, a page-level heading, and no redundant back control. (#247)
