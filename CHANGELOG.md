...
- Derived achievements are now immutable medal events. Completing a first or final topic, answering the 100th question, and passing a simulated exam create a dated `user_medals` row exactly once, show the localized celebration immediately, and remain earned if the question or topic bank later changes. Existing users are not backfilled.
