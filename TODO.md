...
1. **Increase vertical spacing between homepage sections.**
...
2. **Contextual empty states** instead of just showing zeros.
...
3. **Copy consistency pass** - do LAST (touches most strings; every other item adds strings).
...
4. **Replace native JavaScript dialogs with a modal component.**
...
5. **"Today's mission" dead-ends on a completed topic.**
...
6. **Hebrew feedback rendered on the Arabic locale.**
...
7. **Exam question references sign 126 but shows no image.**
...
8. **Mistakes review renders all questions at once.**
...
9. **Exam result copy does not scale with the score.**
...
10. **Emoji hardcoded in the UI despite the no-emoji policy.**
...
11. **Exam answer-review mode keeps leftover exam chrome.**
...
12. **Saving the schedule redirects to Home instead of back to More, with no confirmation.**
    `schedule.js:124-126` redirects to home after 800ms; the user arrives from More (`more/page.tsx:185`) and the page's own back button targets `/more` (`schedule/page.tsx:32`). Fix: redirect to `"/" + window.__locale + "/more"` and add a success confirmation - reuse item 4 by adding a small toast helper to `modal.js` + global CSS (no toast exists today; `--success*` tokens are already defined). Sequence after item 4.

13. **Invalid nested interactive elements on the exam intro.**
...
14. **Daily goal counts distinct questions, not answers given today.**
...
15. **Persist derived achievements to `user_medals`.**
...
1. Isolated/trivial first: 13 (nested interactive), 10 (emoji), 1 (spacing).
2. Foundation: 4 (modal component), then 12 (schedule redirect + toast).
3. Independent features: 6 (locale persistence), 11 (review chrome), 9 (tiered copy), 8 (pagination), 5 (mission picker), 2 (empty states), 7 (sign-126 + migration).
4. Schema-heavy: 14 (daily goal, migration 016+), 15 (medals).
5. Copy pass (3) LAST - it rewrites strings every other item adds.

Conflict hotspots when workspaces overlap: `messages/*.json` (most items), `src/app/[locale]/page.tsx` (1, 2, 5), `public/js/quiz.js` (3, 6, 15), `public/js/exam.js` (4, 9, 11), migration numbering (7, 14, 15 - renumber right before push).
