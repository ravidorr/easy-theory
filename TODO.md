...
Ordered by priority. Items 2–9 are deferred/nice-to-have (formerly the "Later" section) and keep their original order.

## 1. Performance: flashcards & images
...
## 2. Migrate the layout setup to Next 16's root-params / `global-not-found` conventions
...
## 4. Spaced repetition for flashcards/mistakes
...
## 6. Move the hardcoded videos/resources lists into the DB
...
## 7. Clean up one-off scripts and migration hygiene
...
## 8. Migrate `exam/route.ts` to the shared `parseJsonBody` helper
...
## 9. Audit remaining `src/lib/db.ts` helpers that swallow Supabase errors
