...
## 2. Migrate `exam/route.ts` to the shared `parseJsonBody` helper
...
## 3. Audit remaining `src/lib/db.ts` helpers that swallow Supabase errors
...
## 4. Bubbled `notFound()` never reaches `global-not-found` (Next 16.3.0-preview.5)
