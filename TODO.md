...
## 3. Move the hardcoded videos/resources lists into the DB
...
## 4. Migrate `exam/route.ts` to the shared `parseJsonBody` helper
...
## 5. Audit remaining `src/lib/db.ts` helpers that swallow Supabase errors
...
## 6. Bubbled `notFound()` never reaches `global-not-found` (Next 16.3.0-preview.5)
