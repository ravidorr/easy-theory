...
7. **Daily goal counts distinct questions, not answers given today.**
...
8. **Persist derived achievements to `user_medals`.**
...
1. Independent features: 2 (locale persistence), 6 (review chrome), 5 (tiered copy), 4 (pagination), 1 (mission picker), 3 (sign-126 + migration).
2. Schema-heavy: 7 (daily goal, migration 016+), 8 (medals).
