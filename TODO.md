...
1. **Exam result copy does not scale with the score.**
...
2. **Exam answer-review mode keeps leftover exam chrome.**
...
3. **Daily goal counts distinct questions, not answers given today.**
...
4. **Persist derived achievements to `user_medals`.**
...
1. Independent features: 2 (review chrome), 1 (tiered copy).
2. Schema-heavy: 3 (daily goal, migration 017+), 4 (medals).

Conflict hotspots when workspaces overlap: `messages/*.json` (most items), `public/js/quiz.js` (4), `public/js/exam.js` (1, 2), migration numbering (3, 4 - renumber right before push).
