...
1. **Exam question references sign 126 but shows no image.**
...
2. **Mistakes review renders all questions at once.**
...
3. **Exam result copy does not scale with the score.**
...
4. **Exam answer-review mode keeps leftover exam chrome.**
...
5. **Daily goal counts distinct questions, not answers given today.**
...
6. **Persist derived achievements to `user_medals`.**
...
1. Independent features: 4 (review chrome), 3 (tiered copy), 2 (pagination), 1 (sign-126 + migration).
2. Schema-heavy: 5 (daily goal, migration 016+), 6 (medals).

Conflict hotspots when workspaces overlap: `messages/*.json` (most items), `public/js/quiz.js` (6), `public/js/exam.js` (3, 4), migration numbering (1, 5, 6 - renumber right before push).
