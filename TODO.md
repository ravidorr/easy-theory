...
Ordered by priority. All remaining items are deferred/nice-to-have (formerly the "Later" section) and keep their original order.

## 1. Migrate the layout setup to Next 16's root-params / `global-not-found` conventions
...
## 3. Spaced repetition for flashcards/mistakes
...
## 5. Move the hardcoded videos/resources lists into the DB
...
## 6. Clean up one-off scripts and migration hygiene
...
## 7. Migrate `exam/route.ts` to the shared `parseJsonBody` helper
...
## 8. Audit remaining `src/lib/db.ts` helpers that swallow Supabase errors
