...
2. **"Today's mission" dead-ends on a completed topic.**
...
3. **Hebrew feedback rendered on the Arabic locale.**
...
4. **Exam question references sign 126 but shows no image.**
...
5. **Mistakes review renders all questions at once.**
...
6. **Exam result copy does not scale with the score.**
...
7. **Exam answer-review mode keeps leftover exam chrome.**
...
8. **Saving the schedule redirects to Home instead of back to More, with no confirmation.**
...
9. **Daily goal counts distinct questions, not answers given today.**
...
10. **Persist derived achievements to `user_medals`.**
...
1. Foundation: 8 (schedule redirect + toast; extends the shipped modal.js).
2. Independent features: 3 (locale persistence), 7 (review chrome), 6 (tiered copy), 5 (pagination), 2 (mission picker), 4 (sign-126 + migration).
3. Schema-heavy: 9 (daily goal, migration 016+), 10 (medals).
4. Copy pass (1) LAST - it rewrites strings every other item adds.

Conflict hotspots when workspaces overlap: `messages/*.json` (most items), `src/app/[locale]/page.tsx` (2), `public/js/quiz.js` (1, 3, 10), `public/js/exam.js` (6, 7), migration numbering (4, 9, 10 - renumber right before push).
