...
environment:
  ...
  auth:
    ...
    mint: "pnpm qa:mint"
  data_assumptions: "Seeded test DB: 277 signs with images under /signs/; flashcards page loads all of them into #fc-data"
timebox_minutes: 20
out_of_scope:
...
checks:
  ...
  - id: CHK-FC-05
    desc: "The session is fully client-side"
    oracle: "Network log shows no /api/ requests while flipping and marking cards (static /signs/ image fetches are expected)"
  - id: CHK-FC-06
...
- No server persistence: nothing is saved between visits and no `/api/` call should
  fire. Image srcs are validated same-origin with a placeholder fallback
  (`/placeholder.svg`).
