...
describe("videos loading skeleton", () => {
  ...
  it("mirrors the featured video and the video rows", async () => {
    const { container } = render(await Loading());
  ...
  it("keeps the videos tab bar visible", async () => {
    render(await Loading());
    expect(screen.getByTestId("tabbar")).toHaveAttribute("data-active", "videos");
  });
});
